...
import android.kim.automatedtestapp.LocalSVM.svm;
import android.kim.automatedtestapp.LocalSVM.svm_model;
import android.kim.automatedtestapp.LocalSVM.svm_node;
import android.kim.automatedtestapp.LocalSVM.svm_parameter;
import android.kim.automatedtestapp.LocalSVM.svm_problem;
import static android.kim.automatedtestapp.LocalSVM.svm.svm_predict_distance;
...
public class SVMModel {
    ...
    public static final int NUMBEROFSUGGESTIONS = 6;
    ...
    public static svm_model buildModel(List<Integer> ratingsList, List<Float[]> vectorValuesList, List<Integer[]> vectorLabelsList) {
        ...
        //Log.i("home", "svm.java - TestSVM prob: " + problem.x.toString());
        //Log.i("home", "svm.java - TestSVM prob: " + problem.y.toString());
...
//        System.out.println("Elapsed time convertLabelListArray nanosec: " + elapsedTime);
    ...
    private static svm_node[][] buildSparseVectorValuesArray(int numberOfRatings, List<Float[]> vectorValuesList, List<Integer[]> vectorLabelsList ) {
        long startTime = System.nanoTime();
        svm_node[][] probs = new svm_node[numberOfRatings][NUMBEROFHIGHESTPROBS]; //should be 5 right?

        for(int i = 0; i < numberOfRatings; i++)
        {
            svm_node[] svmNodeVector = buildOneSparseVector6(vectorValuesList.get(i), vectorLabelsList.get(i));
            for(int j = 0; j < NUMBEROFHIGHESTPROBS; j++){
...
//        System.out.println("Elapsed time buildSparseVectorValuesArray nanosec: " + elapsedTime);
    ...
    //Used in create SVM model
    // build a vector based on all 1000 features
    private static svm_node[] buildOneSparseVector6(Float[] probsLists, Integer[] labelList){
        long startTime = System.nanoTime();
        svm_node[] svmNodeArray = new svm_node[NUMBEROFHIGHESTPROBS];
        for (int i = 0; i < NUMBEROFHIGHESTPROBS; i++){
            //Log.i(TAG, "size label list " + labelList.length);
            //Log.i(TAG, "size probs list " + probsLists.length);
...
//        System.out.println("Elapsed time buildOneSparseVector nanosec: " + elapsedTime);
    ...
    private static svm_node[] buildOneSparseVector(String bestProbsString, String bestLabelsString) {
        ...
        svm_node[] svmNodeArray = new svm_node[NUMBEROFSUGGESTIONS]; // RESULTS_TO_SHOW = 6
        for (int i = 0; i < 6; i++){
    ...
    private static float[] getFloatValues(String values) {
        String[] stringValues = values.split(" ");
        float[] floatValues = new float[NUMBEROFSUGGESTIONS];
    ...
    public static svm_node buildOneIndexLabelEntry(int index, double x) {
        //Log.i("SVM ", "build one index label entry was called with index " + index + "value " + x);
        long startTime = System.nanoTime();
...
//        System.out.println("Elapsed time buildOneIndexLabelEntry nanosec: " + elapsedTime);
    ...
    public static double doPredictionDistanceBased(svm_model model, Float[] probsLists, Integer[] labelList) {
        long startTime = System.nanoTime();
